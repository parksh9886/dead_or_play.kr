...
export default function AdminPage() {
  ...
  const createNextRound = async () => {
    ...
    const { error } = await supabase.from('game_rounds').insert({
      ...
      description: newDesc,
      game_type: gameType,
      config: config, // JSON으로 자동 변환되어 들어감
      status: 'ACTIVE'
    });

    if (!error) {
      alert("생성 완료!");
      setNewTitle(""); setOptionA(""); setOptionB(""); setQuizPlaceholder("");
      fetchRounds();
    } else {
      alert(error.message);
    }
    ...
    <div className="min-h-screen bg-black text-white p-8 pb-32">
      ...
      <div className="bg-gray-900 p-6 rounded-xl border border-gray-700 mb-12">
        ...
        <div className="grid gap-4">
          ...
          {gameType === 'QUIZ' && (
            <input className="bg-gray-800 p-3 rounded" placeholder="입력창 안내문구 (예: 정답을 띄어쓰기 없이 입력)" value={quizPlaceholder} onChange={e=>setQuizPlaceholder(e.target.value)} />
          )}
               ...
               <div className="mt-4 flex gap-2">
                 ...
                 {round.game_type === 'VOTE' ? (
                   ...
                   <div className="flex w-full gap-2">
                     <input id={`ans-${round.id}`} className="bg-black p-2 rounded flex-1" placeholder="정답 텍스트 입력" />
                     <button onClick={() => {
                       const val = (document.getElementById(`ans-${round.id}`) as HTMLInputElement).value;
                       if(val) closeRound(round.id, 'QUIZ', val);
                     }} className="bg-red-600 px-4 rounded font-bold">정답 확정 & 종료</button>
                   </div>
                 )}
               </div>
            )}
